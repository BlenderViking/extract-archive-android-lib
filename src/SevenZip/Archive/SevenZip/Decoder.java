...
class Decoder {
	...
	int Decode(SevenZip.IInStream inStream, long startPos,
			LongVector packSizes,
			int packSizesOffset, // const UInt64 *packSizes,
			Folder folderInfo, java.io.OutputStream outStream,
			SevenZip.ICompressProgressInfo compressProgress // , //
															// ICompressProgressInfo
															// *compressProgress,
	// _ST_MODE boolean mtMode, int numThreads
	) throws IOException {

		ObjectVector<java.io.InputStream> inStreams = new ObjectVector<java.io.InputStream>(); // CObjectVector<
		...
		if (createNewCoders) {
			...
			for (i = 0; i < numCoders; i++) {
				 ...
				 * (!GetMethodInfo(altCoderInfo.MethodID, methodInfo)) return
				 * E_NOTIMPL; #endif
				 */
				...
				if (coderInfo.IsSimpleCoder()) {
					...
					if (altCoderInfo.MethodID.equals(MethodID.k_PPMD))
						System.out.println("PPMD not implemented"); // decoder =
																	// new
		...
		for (i = 0; i < numCoders; i++) {
			CoderInfo coderInfo = folderInfo.Coders.get(i);
			AltCoderInfo altCoderInfo = coderInfo.AltCoders.Front();
			Object decoder = _decoders.get(coderIndex); // CMyComPtr<IUnknown>
														// &decoder =
...

			{
				try {
					...
					if (size == -1) // (size > 0xFFFFFFFF)
						return HRESULT.E_NOTIMPL;
					if (size > 0) {
						boolean ret = setDecoderProperties
			 ...
			 * 
			 * #ifndef _NO_CRYPTO { CMyComPtr<ICryptoSetPassword>
			 * cryptoSetPassword; decoder.QueryInterface(IID_ICryptoSetPassword,
			 * &cryptoSetPassword); if (cryptoSetPassword) { if (getTextPassword
			 * == 0) return E_FAIL; CMyComBSTR password;
			 * RINOK(getTextPassword->CryptoGetTextPassword(&password));
			 * CByteBuffer buffer; UString unicodePassword(password); const
			 * UInt32 sizeInBytes = unicodePassword.Length() * 2;
			 * buffer.SetCapacity(sizeInBytes); for (int i = 0; i <
			 * unicodePassword.Length(); i++) { wchar_t c = unicodePassword[i];
			 * ((Byte *)buffer)[i * 2] = (Byte)c; ((Byte *)buffer)[i * 2 + 1] =
			 * (Byte)(c >> 8); } RINOK(cryptoSetPassword->CryptoSetPassword(
			 * (const Byte *)buffer, sizeInBytes)); } } #endif
			 */
			coderIndex++;

			int numInStreams = coderInfo.NumInStreams;
			int numOutStreams = coderInfo.NumOutStreams;
			LongVector packSizesPointers = new LongVector(); // CRecordVector<const
																// UInt64 *>
			LongVector unPackSizesPointers = new LongVector(); // CRecordVector<const
																// UInt64 *>
			packSizesPointers.Reserve(numInStreams);
			unPackSizesPointers.Reserve(numOutStreams);
			int j;
			for (j = 0; j < numOutStreams; j++, unPackStreamIndex++)
				unPackSizesPointers.add(folderInfo.UnPackSizes
						.get(unPackStreamIndex));

			for (j = 0; j < numInStreams; j++, packStreamIndex++) {
				int bindPairIndex = folderInfo
						.FindBindPairForInStream(packStreamIndex);
				if (bindPairIndex >= 0)
					packSizesPointers
							.add(folderInfo.UnPackSizes
									.get(folderInfo.BindPairs
											.get(bindPairIndex).OutIndex));
				else {
					int index = folderInfo
