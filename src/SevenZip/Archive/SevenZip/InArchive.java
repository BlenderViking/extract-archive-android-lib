...
class InArchive extends Header {
	...
	static boolean TestSignatureCandidate(byte[] testBytes, int off) {
		for (int i = 0; i < kSignatureSize; i++) {
			// System.out.println(" " + i + ":" + testBytes[i] + " " +
			// kSignature[i]);
	...
	int ReadNum() throws IOException { // CNum
		...
		if (value64 > InArchive.kNumMax)
			throw new IOException("ReadNum - value > CNum.kNumMax"); // return
																		// E_FAIL;
	...
	int ReadStreamsInfo(ObjectVector<ByteBuffer> dataVector, long[] dataOffset,
			LongVector packSizes, BoolVector packCRCsDefined,
			IntVector packCRCs, ObjectVector<Folder> folders,
				...
				for (int i = 0; i < folders.size(); i++) {
					...
					if (numSubstreams == 1 && folder.UnPackCRCDefined) {
						...
						for (int j = 0; j < numSubstreams; j++, digestIndex++) {
							digestsDefined
									.add(digestsDefined2.get(digestIndex));
	...
	int ReadAndDecodePackedStreams(long baseOffset, long[] dataOffset,
			ObjectVector<ByteBuffer> dataVector // CObjectVector<CByteBuffer>
												// &dataVector
	) throws IOException {
		LongVector packSizes = new LongVector(); // CRecordVector<UInt64>
													// packSizes;
		BoolVector packCRCsDefined = new BoolVector(); // CRecordVector<bool>
														// packCRCsDefined;
		...
		ReadStreamsInfo(null, dataOffset, packSizes, packCRCsDefined, packCRCs,
				folders, numUnPackStreamsInFolders, unPackSizes,
		...
		for (int i = 0; i < folders.size(); i++) {
			Folder folder = folders.get(i); // const CFolder &folder =
											// folders[i];
	...
	int ReadDatabase(ArchiveDatabaseEx database) throws IOException {
		...
		ObjectVector<ByteBuffer> dataVector = new ObjectVector<ByteBuffer>(); // CObjectVector<CByteBuffer>
																				// dataVector;
		...
		for (;;) {
			...
			int result = ReadAndDecodePackedStreams(
					database.ArchiveInfo.StartPositionAfterHeader, ltmp, // database.ArchiveInfo.DataStartPosition2,
					dataVector);
			if (result != HRESULT.S_OK)
				return result;
