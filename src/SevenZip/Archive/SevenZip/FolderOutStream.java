...
class FolderOutStream extends java.io.OutputStream {
	...
	int OpenFile() throws java.io.IOException {
		...
		// RINOK(_extractCallback->GetStream(_ref2Offset + index,
		// &realOutStream, askMode));
	...
	int WriteEmptyFiles() throws java.io.IOException {
		for (; _currentIndex < _extractStatuses.size(); _currentIndex++) {
			...
			res = _extractCallback
					.SetOperationResult(IInArchive.NExtract_NOperationResult_kOK);
			if (res != HRESULT.S_OK)
				return res;
			...
			throws java.io.IOException // UInt32 *processedSize
	{
		...
		while (_currentIndex < _extractStatuses.size()) {
			if (_fileIsOpen) {
				...
				// java.io.IOException("_outStreamWithHash.Write : " + res); //
				// return res;
				...
				if (_filePos == fileSize) {
					...
					if (fileInfo.IsFileCRCDefined)
						digestsAreEqual = (fileInfo.FileCRC == _outStreamWithHashSpec
								.GetCRC());
					else
						digestsAreEqual = true;

					int res = _extractCallback
					...
					if (res != HRESULT.S_OK)
						throw new java.io.IOException(
								"_extractCallback.SetOperationResult : " + res); // return
																					// res;
				...
				int res = OpenFile();
				if (res != HRESULT.S_OK)
					throw new java.io.IOException("OpenFile : " + res); // return
																		// res;
				_fileIsOpen = true;
				_filePos = 0;
			}
