...
public class CoderMixer2ST implements ICompressCoder2, CoderMixer2 {
	...
	public int GetInStream(RecordVector<java.io.InputStream> inStreams,
		...
		ICompressSetInStream setInStream = (ICompressSetInStream) coder.Coder; // coder.Coder.QueryInterface(IID_ICompressSetInStream,
																				// &setInStream);
		if (setInStream == null)
			return HRESULT.E_NOTIMPL;
	...
	public int Code(
		...
		ObjectVector<java.io.InputStream> seqInStreams = new ObjectVector<java.io.InputStream>(); // CObjectVector<
																									// CMyComPtr<ISequentialInStream>
																									// >
		ObjectVector<java.io.OutputStream> seqOutStreams = new ObjectVector<java.io.OutputStream>(); // CObjectVector<
		...
		for (i = 0; i < mainCoder.NumOutStreams; i++) {
			java.io.OutputStream tmp[] = new java.io.OutputStream[1];
			int res = GetOutStream(outStreams, useless_outSizes, startOutIndex
					+ i, tmp);
			if (res != HRESULT.S_OK)
				return res;
		...
		for (i = 0; i < _coders.size(); i++) {
			...
			try {
				...
				int res = setOutStreamSize
						.SetOutStreamSize(coder.OutSizePointers.get(0));
				if (res != HRESULT.S_OK)
					return res;
			} catch (java.lang.ClassCastException e) {
				// nothing to do
