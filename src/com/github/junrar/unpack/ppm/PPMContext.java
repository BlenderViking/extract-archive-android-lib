...
public class PPMContext extends Pointer {
	...
	public final static int[] ExpEscape = { 25, 14, 9, 7, 5, 5, 4, 4, 4, 3, 3,
			3, 2, 2, 2, 2 };
	...
	public void rescale(ModelPPM model) {
		...
		do {
			...
			if (p.getFreq() > temp.getFreq()) {
				...
				} while (p1.getAddress() != freqData.getStats()
						&& tmp.getFreq() > temp3.getFreq());
				p1.setValues(tmp);
			}
		} while (--i != 0);
		if (p.getFreq() == 0) {
			...
			if (getNumStats() == 1) {
				...
				model.getSubAlloc().freeUnits(freqData.getStats(),
						(OldNS + 1) >>> 1);
		...
		if (n0 != n1) {
			freqData.setStats(model.getSubAlloc().shrinkUnits(
					freqData.getStats(), n0, n1));
		}
	...
	public boolean decodeSymbol1(ModelPPM model) {
		...
		if (count < (HiCnt = p.getFreq())) {
			coder.getSubRange().setHighCount(HiCnt);
			model.setPrevSuccess((2 * HiCnt > coder.getSubRange().getScale()) ? 1
					: 0);
		...
		while ((HiCnt += p.incAddress().getFreq()) <= count) {
			if (--i == 0) {
				...
				model.getFoundState().setAddress(0);
				do {
					model.getCharMask()[p.decAddress().getSymbol()] = model
							.getEscCount();
				} while (--i != 0);
				coder.getSubRange()
						.setHighCount(coder.getSubRange().getScale());
				return (true);
			}
		}
		coder.getSubRange().setLowCount(HiCnt - p.getFreq());
